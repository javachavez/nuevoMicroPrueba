package com.jumbotours.web.rest;
...
import com.jumbotours.config.KafkaProperties;
...
class NuevoMicroPruebaKafkaResourceIT {
    ...
    void setup() {
        ...
        NuevoMicroPruebaKafkaResource kafkaResource = new NuevoMicroPruebaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/nuevo-micro-prueba-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/nuevo-micro-prueba-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
